...
class DistanceInfoViewController: UIViewController {
    ...
    let MoonDistanceInches = 15136700000
    ...
    override func viewDidLoad() {
        ...
        var daysToTravel = round(10*(Double(MoonDistanceInches)/inchesPerDay)/10)
        var days = Int(floor(daysToTravel))
        //var remainingHours = (daysToTravel*10).remainder(dividingBy: 10)
        //var hours = Int((remainingHours/10) * 24)
        moonDistanceLabel.text = "\(days)d"
        
        daysToTravel = round(10*(Double(DisneyDistanceInches)/inchesPerDay)/10)
        days = Int(floor(daysToTravel))
        var remainingHours = (daysToTravel*10).remainder(dividingBy: 10)
        var hours = Int((remainingHours/10) * 24)
        disneyDistanceLabel.text = "\(days)d \(hours)h"
        
        daysToTravel = round(10*(Double(DamDistanceInches)/inchesPerDay)/10)
        days = Int(floor(daysToTravel))
        remainingHours = (daysToTravel*10).remainder(dividingBy: 10)
        hours = Int((remainingHours/10) * 24)
        damDistanceLabel.text = "\(days)d \(hours)h"
        
        daysToTravel = round(10*(Double(TSUDistanceInches)/inchesPerDay)/10)
        days = Int(floor(daysToTravel))
        remainingHours = (daysToTravel*10).remainder(dividingBy: 10)
        hours = Int((remainingHours/10) * 24)
        TSUDistanceLabel.text = "\(days)d \(hours)h"
        
        daysToTravel = round(10*(Double(KentuckyDistanceInches)/inchesPerDay)/10)
        days = Int(floor(daysToTravel))
        remainingHours = (daysToTravel*10).remainder(dividingBy: 10)
        hours = Int((remainingHours/10) * 24)
